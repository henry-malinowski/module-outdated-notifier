...
export default class SettingsConfig extends HandlebarsApplicationMixin(ApplicationV2) {
      ...
      buttons: [
        {
          type: "submit",
          icon: "fas fa-save",
          label: "module-outdated-notifier.settingsConfig.button.save",
        },
      ],
    };
  }
  ...
  async #processFile(file) {
    if (file.name !== "license.mjs") {
      ui.notifications.warn("module-outdated-notifier.settingsConfig.messages.fileWrongName", {
        localize: true,
      });
      return;
    }
    ...
    if (match && match[1]) {
      ...
      await this.render();
      ui.notifications.info("module-outdated-notifier.settingsConfig.messages.extractionSuccess", {
        localize: true,
      });
    } else {
      ui.notifications.error("module-outdated-notifier.settingsConfig.messages.extractionError", {
        localize: true,
      });
    }
  }
